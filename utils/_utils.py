...
def all_subsets(cols: List,
    ...
    ----------
    cols : List
        a list of items to create combinations from
    k_range : Tuple[int, int]
        a tuple of two integers representing the inclusive lower and upper bounds for the length of the subsets to
        be generated

    Returns
    -------
    Iterator:
        An iterator that yields all possible combinations of the columns in `cols` with length `k` for
        all `k` in the range of `k_range`.
    """
...
def get_ranks(arr: Union[List[float], np.ndarray],
    ...
    normalize : bool
        If True, the ranks will be normalized so that they sum to 1. Defaults to False.
