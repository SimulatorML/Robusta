...
def get_col_score(estimator: BaseEstimator,
    ...
    ----------
    estimator : BaseEstimator
        The estimator to use for scoring.
    X : pd.DataFrame
        The input data of shape (n_samples, n_features).
    y : pd.Series
        The target data of shape (n_samples,).
    col : str
        The name of the column to permute.
    n_repeats : int
        The number of times to repeat the permutation. Default is 5.
    scoring : Optional[str, Callable]
        The scoring metric to use. Default is None, which uses the estimator's default scorer.
    random_state : Optional[int]
        The random seed to use. Default is None, which uses the global numpy random number generator.

    Returns
    -------
    scores : np.ndarray
        The scores when `col` is permuted, of length `n_repeats`.
    """
...
class PermutationImportance(BaseEstimator, MetaEstimatorMixin):
    ...
    def score(self,
        ...
        **score_params: dict
        ...
        -------
        score : float
            Score of the wrapped estimator on the given data.
        """
