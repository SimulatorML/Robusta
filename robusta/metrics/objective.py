...
def blend_objective(y_true: np.array,
    ...
    # Calculate gradient and Hessian for first loss function
    c = 0.5
    grad_l1 = c * residual / (abs_residual + c)
    hess_l1 = c ** 2 / (abs_residual + c) ** 2
    ...
    grad = coef[0] * grad_l1 + coef[1] * grad_huber + coef[2] * residual + coef[3] * grad_mae
    hess = coef[0] * hess_l1 + coef[1] * hess_huber + coef[2] * hess_rmse + coef[3] * hess_mae
