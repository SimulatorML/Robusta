...
class DividedOutlierDetector(BaseEstimator, OutlierMixin):
    ...
    def fit_resample(self,
        ...
        # Use the Parallel function to run the delayed jobs in parallel and get the paths of outliers and outliers for each cluster
        paths = Parallel(backend='multiprocessing', max_nbytes='256M', pre_dispatch='all',
                         verbose=self.verbose, n_jobs=self.n_jobs)(jobs)

        # Unzip the paths to get the outliers and outliers for each cluster, as well as the corresponding detector objects
...
def od_path(detector: object,
    ...
    Args:
    detector (object): Outlier detection algorithm
    X (pd.DataFrame): Feature matrix of shape (n_samples, n_features)
    y (pd.Series): Label vector of shape (n_samples,)
    ind (np.ndarray): Indices of the subset of data to perform outlier detection on

    Returns:
    Tuple[np.ndarray, np.ndarray, LocalOutlierFactor]: Subset of X and y without outliers, and the detector used
    """
