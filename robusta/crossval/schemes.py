...
def shuffle_labels(labels: pd.Series,
    ...
    Args:
        labels (pd.Series): The list containing the labels to be shuffled.
        random_state (int, optional): Seed or state for the random number generator.
            Default is None, which uses the global numpy random number generator.

    Returns:
        pd.Series: A new pandas Series object with the same values as the original labels,
            but with the values shuffled using the random state specified.

    Raises:
        TypeError: If the input `labels` is not a pandas Series.
        ValueError: If the input `labels` contains non-unique values.
    """
    ...
    # Check if labels has unique values
    if not labels.is_unique:
        raise ValueError("Input 'labels' must contain unique values.")
...
class AdversarialValidation:
    ...
    Methods
    -------
    split(X: pd.DataFrame) -> Generator[Tuple[int, int], None, None]:
    ...
    def _get_train_size(self,
        ...
        Returns:
        --------
        int : The size of the training set.
        """
...
def make_adversarial_validation(classifier: ClassifierMixin,
    ...
    Args:
        classifier: A pre-fitted binary classifier (e.g. a logistic regression or a random forest classifier).
        X_train: The training data as a pandas DataFrame.
        X_test: The validation data as a pandas DataFrame.
        train_size: The proportion of the combined dataset to be used as the training set. This can be a float (between
        ...
        test_size: The proportion of the combined dataset to be used as the validation set. This can be a float (between
    ...
    Raises:
        ValueError: If both train_size and test_size are passed.
    """
