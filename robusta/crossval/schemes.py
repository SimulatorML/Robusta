...
def shuffle_labels(labels: pd.Series, random_state: Optional[int] = None) -> pd.Series:
    """
    ...
    random_state : int, optional
        Seed or state for the random number generator. Default is None, which uses the global numpy random
        number generator.

    Returns
    -------
    pd.Series:
        A new pandas Series object with the same values as the original labels, but with the values shuffled using
        the random state specified.
...
class RepeatedGroupKFold:
    ...
    def __init__(self, n_splits: int = 5, n_repeats: int = 3, random_state: int = 0):
    ...
    def split(
        ...
        for i in range(self.n_repeats):
            ...
            groups_i = shuffle_labels(labels=groups, random_state=self.random_state + i)
            ...
            split = cv.split(X=X, y=y, groups=groups_i)
...
class RepeatedKFold:
    ...
    def __init__(
        self, n_splits: int = 5, n_repeats: int = 3, random_state: Optional[int] = None
    ):
        self.n_splits = n_splits
        self.n_repeats = n_repeats
        self.random_state = random_state

    def split(self, X: pd.DataFrame, y: pd.Series) -> Tuple[np.ndarray, np.ndarray]:
        """
...
class StratifiedGroupKFold:
    ...
    def split(self, y: pd.Series, groups: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """
...
class AdversarialValidation:
    ...
    def _get_train_size(self, X: pd.DataFrame) -> int:
        """
        ...
        if train_size is not None:
            if isinstance(train_size, float):
                if 0 < train_size < 1:
                    ...
                    raise ValueError(
                        "Float train_size must be in range (0, 1). "
                ...
                if 0 < train_size < size:
                    ...
                    raise ValueError(
                        "Integer train_size must be in range [1, {}]. "
                        "Passed {}".format(size, train_size)
                    )
            else:
                raise ValueError(
                    "Unknown type of train_size passed {}".format(train_size)
                )
        ...
        if test_size is not None:
            if isinstance(test_size, float):
                ...
                if 0 < test_size < 1:
                    ...
                    raise ValueError(
                        "Float test_size must be in range (0, 1). "
                ...
                if 0 < test_size < size:
                    ...
                    raise ValueError(
                        "Integer test_size must be in range [1, {}]. "
                        "Passed {}".format(size, test_size)
                    )
            else:
                raise ValueError(
                    "Unknown type of test_size passed {}".format(test_size)
                )


def make_adversarial_validation(
    classifier: ClassifierMixin,
    ...
    train_size : int, float or optional
    ...
    test_size : int, float or optional
    ...
    return AdversarialValidation(
        classifier=clf, train_size=train_size, test_size=test_size
    )
