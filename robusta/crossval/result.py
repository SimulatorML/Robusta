...
def argsort_idx(idx_list: np.ndarray) -> np.ndarray:
    ...
    Args:
    - idx_list: a array of indices in string or integer format

    Returns:
    - sorted_indices: a numpy array of indices that would sort the input list
    """
...
def check_cvs(results: List[Dict[str, Any]],
              X: pd.DataFrame,
              y: pd.Series,
              groups: Optional[pd.Series] = None) -> Any:
    """
    ...
    for cv_i in cvs[1:]:
        if cv0 != cv_i:
            ...
            for (trn0, oof0), (trn_i, oof_i) in zip(cv0.split(X, y, groups), cv_i.split(X, y, groups)):
...
def load_results(idx_list: Optional[List[int]] = None,
                 result_path: str = './results/',
                 raise_error: bool = False) -> Dict[Any, dict[str, int | str | object] | None]:
    """
...
def split_cv_groups(results: Dict[str, Any],
                    y_train: List[Any]) -> List[List[str]]:
    """
    Splits cross-validation (CV) results into groups with the same CV fold assignments.

    Args:
        results (dict): A dictionary mapping string keys to dictionaries that contain cross-validation results.
                        Each dictionary must contain a key 'cv' that maps to a scikit-learn BaseCrossValidator
                        object. Other keys can be present but are not used in this function.
...
def check_folds(results: Dict[str, Any],
                y_train: List[Any]) -> Dict[int, List[int]]:
    """
    Check the cross-validation schemes used in a machine learning model.

    Args:
        results (Dict[str, Any]): A dictionary containing the results of a machine learning model's cross-validation.
        y_train (List[Any]): A list of the target values for the training data.

    Returns:
        Dict[int, List[int]]: A dictionary mapping the index of each cross-validation scheme to a list of the indices of the
                              training data used in that scheme.
    """

    # Split the cross-validation groups using the `split_cv_groups` function
    cv_groups = split_cv_groups(results=results,
                                y_train=y_train)
...
def rating_table(results: Dict[str, Dict[str, float]],
                 n_digits: int = 4,
                 fold_scores: bool = False) -> pd.DataFrame:
    """
    Creates a table containing the performance metrics of different models on a dataset.

    Args:
        results: A dictionary containing the performance metrics for different models.
        ...
        n_digits: (int) The number of digits to round the scores to.
        fold_scores: (bool) Whether to include the validation scores for different folds in the output.
