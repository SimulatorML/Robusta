...
def _fit_predict(
    ...
    X_trn, y_trn = _safe_split(estimator=estimator, X=X, y=y, indices=trn)

    # split the out-of-fold data and labels using the provided indices
    X_oof, y_oof = _safe_split(estimator=estimator, X=X, y=y, indices=oof)
    ...
    if return_pred and (len(oof) or len(new)):
        ...
        if len(oof):
            oof_pred = _predict(
                estimator=estimator, method=method, X=X_oof, y=y, index=oof_index
            )
            result["oof_pred"] = oof_pred
        if len(new):
            new_pred = _predict(
                estimator=estimator, method=method, X=X_new, y=y, index=new_index
            )
...
def _predict(
    ...
    # If the estimator is not a classifier, raise a TypeError
        ...
        raise TypeError(
            "<{}> is an estimator of unknown type: \
                         <{}>".format(
                name, est_type
            )
        )
...
def _check_avg(
    estimator: BaseEstimator, avg_type: str, method: str
) -> Tuple[Callable, str]:
    """
    ...
    if method not in methods:
        ...
        raise ValueError(
            "<method> should be in {}".format(set(methods))
    ...
    if avg_type not in avg_types:
        ...
        raise ValueError(
            "<avg_type> should be in {}".format(set(avg_types))
    ...
    if is_classifier(estimator) and hasattr(estimator, "predict_proba"):
        if method is "predict_proba":
            ...
            if avg_type is "pass":
                ...
                bad_vals = {"soft", "hard"}
                msg = (
                    "Selected <method> value is {}.".format(method)
            ...
            if avg_type in ["soft", "auto"]:
                ...
                raise ValueError(
                    "Passed unavailable avg_type '{}' for method <{}>"
    ...
    # Check if the estimator is a classifier and does not have the predict_proba attribute
        ...
        if method is "predict_proba":
            ...
            if avg_type in ["hard", "auto"]:
                ...
                vals = {"auto", "hard", "pass"}
                msg = (
                    "<{}> is a {}. ".format(name, "non-probabilistic classifier")
...
def _drop_zero_class(
    pred: Union[pd.DataFrame, pd.Series], y: pd.Series
    ...
    elif list(pred.columns) == ["0", "1"] or list(pred.columns) == [0, 1]:
        pred = pred.iloc[:, 1]
        pred.name = y.name
...
def _mean_pred(pred: Union[pd.DataFrame, pd.Series]) -> Union[pd.DataFrame, pd.Series]:
    ...
    if hasattr(pred.columns, "levels"):
        return _multioutput_pred(pred=pred, avg=_mean_pred)
...
def _rank_pred(pred: Union[pd.DataFrame, pd.Series]) -> Union[pd.DataFrame, pd.Series]:
    ...
    # If the input has multiple targets, call this function recursively on each target
    if hasattr(pred.columns, "levels"):
        return _multioutput_pred(pred=pred, avg=_rank_pred)
...
def _soft_vote(pred: Union[pd.DataFrame, pd.Series]) -> DataFrame | Any:
    ...
    # If the input has multiple targets, call this function recursively on each target
    if hasattr(pred.columns, "levels"):
        return _multioutput_vote(pred=pred, vote=_soft_vote)
...
def _hard_vote(pred: Union[pd.DataFrame, pd.Series]) -> DataFrame | Any:
    ...
    # If the input has multiple targets, call this function recursively on each target
    if hasattr(pred.columns, "levels"):
        return _multioutput_vote(pred=pred, vote=_hard_vote)
...
def _multioutput_vote(pred: pd.DataFrame, vote: Callable) -> pd.DataFrame:
    """
...
def _multioutput_pred(pred: pd.DataFrame, avg: Callable) -> pd.DataFrame:
    """
