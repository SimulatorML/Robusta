...
from ._curve import _cat_staged_predict, _lgb_staged_predict, _xgb_staged_predict
...
def plot_learning_curve(
    ...
    ----------
    result : dict
        The result dictionary obtained from a call to a `scikit-learn` `GridSearchCV` or `RandomizedSearchCV` object.
    X : pd.DataFrame
        The feature matrix.
    y : pd.Series
        The target variable.
    groups : pd.Series, optional
        The groups used for grouping samples in the `cv.split` method of the cross-validator. Defaults to None.
    max_iter : int, optional
        The maximum number of iterations to plot. If 0, the maximum number of iterations is determined by the number
        of iterations of the estimators. Defaults to 0.
    step : int, optional
        The step size between iterations. Defaults to 1.
    mode : str, optional
        The mode of the plot, either 'mean', 'fold', or 'both'. 'mean' plots the mean score across all folds,
        'fold' plots the score of each individual fold, and 'both' plots both. Defaults to 'mean'.
    train_score : bool, optional
        Whether to plot the training score in addition to the validation score. Defaults to False.
    n_jobs : int, optional
        The number of jobs to run in parallel for computing the scores. If -1, use all available CPUs. Defaults to None.

    Returns
    -------
    Tuple[np.ndarray, np.ndarray]:
        A tuple containing two arrays: the training scores (shape = (n_folds, n_iterations)) and the validation scores
        (shape = (n_folds, n_iterations)).

    Raises
    ------
    AssertionError:
        If the specified `mode` is not one of 'mean', 'fold', or 'both'.
    NotImplementedError:
    ...
    if not train_score:
        ...
        if mode in ["mean", "both"]:
            plt.fill_between(stages, avg - std, avg + std, alpha=0.1, color="b")
    ...
    if True:
        ...
        if mode in ["mean", "both"]:
            plt.fill_between(stages, avg - std, avg + std, alpha=0.1, color="y")
...
def plot_ttest(
    ...
    labels = ["0", "1"]
    if label in resultA:
        labels[0] = resultA[label]
    if label in resultB:
        labels[1] = resultB[label]
    ...
    ax.legend(["t-value = {:.4f}".format(t), "p-value = {:.4f}".format(p)])
    ax.grid(alpha=0.2)
...
def plot_roc_auc(
    ...
    ----------
    results (Union[List, Dict]):
        A list or dictionary of dictionaries, where each dictionary has an 'estimator' key pointing to a trained
        classifier that has a 'predict_proba' method. If a list, each estimator is assumed to use the same
        cross-validation object, otherwise each dictionary must contain a 'cv' key pointing
        to a cross-validation object.
    X : pd.DataFrame
        The input data used for training the classifiers.
    y : pd.Series
        The target data used for training the classifiers.
    groups : Optional[pd.Series]
        An array containing group labels for the samples, used for group-based cross-validation. Default is None.
    labels : Optional[List]
        A list of strings to label the ROC curve for each classifier. If None, the label for each classifier
        will be an integer based on its index. Default is None.
    colors : Optional[List]
        A list of color strings to use for each ROC curve. If None, a default color cycle is used. Default is None.
    steps : int
        The number of steps to use when interpolating the ROC curve. Default is 200.
    ...
    ------
    AssertionError:
        If the labels or colors lists are not the same length as the number of classifiers in the results list.
        Also raised if any of the results dictionaries do not contain an 'estimator' key pointing to
        a classifier object.
    """

    # check cross-validation strategy and generate a cross-validator object
    cv = check_cvs(results=results, X=X, y=y, groups=groups)
    ...
    for label, tprs in curves.items():
        ...
        plt.fill_between(avg_fpr, tpr_lower, tpr_upper, color=c, alpha=0.1)
