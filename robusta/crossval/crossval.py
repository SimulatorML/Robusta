...
def copy(estimator: BaseEstimator) -> object | Any:
    ...
    If the estimator object has a 'copy' method, this method is called to create a copy.
    Otherwise, the 'clone' function is used to create a copy of the estimator object.
    """
...
def crossval(
    ...
    **kwargs
    ...
    avg, method = _check_avg(estimator=estimator, avg_type=avg_type, method=method)
    ...
    logger = CVLogger(
        estimator=estimator, cv=cv, verbose=verbose, n_digits=n_digits, compact=compact
    )
    ...
    parallel = Parallel(
        max_nbytes="256M", pre_dispatch="2*n_jobs", n_jobs=n_jobs, require="sharedmem"
    )
    ...
    if np.any(np.in1d(needs_concat, list(result))):
        ...
        if "oof_pred" in result:
            ...
            oof_pred = _avg_preds(preds=oof_preds, avg=avg, X=X, y=y, index=y.index)
        ...
        if "new_pred" in result:
            ...
            new_pred = _avg_preds(
                preds=new_preds, avg=avg, X=X_new, y=y, index=new_index
            )
    ...
    if hasattr(X, "columns"):
        result["features"] = list(X.columns.values)
