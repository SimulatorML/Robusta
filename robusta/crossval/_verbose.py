...
class CVLogger:
    ...
    def log(self, fold: int, result: Dict[str, float]) -> None:
    ...
    def end(self, result: Dict[str, float]) -> None:
        ...
        if self.compact:
            ...
            if self.verbose >= 1:
                # Print the mean and standard deviation of training scores if available
                if "trn_score" in result:
                    self._log(f" TRAIN:   {mt} ± {st}")
                # Print the mean and standard deviation of validation scores if available
                if "val_score" in result:
                    self._log(f" VALID:   {mv} ± {sv}")
                # Print a newline character
                print()

    def _log(self, msg: str, end: str = " " * 4) -> None:
