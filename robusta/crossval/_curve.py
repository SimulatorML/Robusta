...
def _xgb_staged_predict(
    estimator: XGBRegressor | XGBClassifier, X: pd.DataFrame, max_iter: int = 0
) -> None:
    """
...
def _lgb_staged_predict(
    estimator: LGBMRegressor | LGBMClassifier,
    X: pd.DataFrame,
    max_iter: int = 0,
    step: int = 1,
) -> np.ndarray:
    ...
    Parameters
    ----------
    estimator : lightgbm.LGBMClassifier or lightgbm.LGBMRegressor
        The fitted LightGBM estimator.
    X : ndarray
        The input data for which to make predictions.
    max_iter : int
        The maximum number of stages to generate predictions for. If set to 0 (default), generates predictions
        for all stages.
    step : int
        The step size between stages for which to generate predictions. Default is 1.
...
def _cat_staged_predict(
    estimator: CatBoostRegressor | CatBoostClassifier,
    X: pd.DataFrame,
    max_iter: int = 0,
    step: int = 1,
) -> Generator:
...
class _StagedClassifier(BaseEstimator):
    ...
    def predict(X: np.ndarray) -> np.ndarray:
        ...
        -------
        np.ndarray:
            The predicted labels, which are 1 if the probability of the positive class is greater than 0.5
            and 0 otherwise.
