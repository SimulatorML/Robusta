...
class _Selector(BaseEstimator, TransformerMixin):
    ...
    def _set_features(self, X: pd.DataFrame) -> None:
        """
...
class _WrappedSelector(_Selector):
    ...
    def min_features_(self) -> int:
        ...
        """
        min_features = _check_k_features(
            self.min_features, self.n_features_, "min_features"
        )
        return min_features
    ...
    def max_features_(self) -> int:
        ...
        """
        max_features = _check_k_features(
            self.max_features, self.n_features_, "max_features"
        )
    ...
    @staticmethod
    def _get_importance(subset: List[str], result: Dict[str, Any]):
        """
    ...
    def _eval_subset(
        self, subset: List[int], X: pd.DataFrame, y: pd.Series, groups: np.ndarray
    ) -> List[str]:
        """
    ...
    def eval_subset(
        ...
        # Update stats
        self.total_time_ = getattr(self, "total_time_", 0.0) + subset.eval_time
...
def _check_k_features(
    k_features: Union[int, float], n_features: int, param: str = "k_features"
) -> int:
    """
    ...
    if isinstance(k_features, int):
        ...
        raise ValueError(
            f"Parameter <{param}> must be int or float," f"got {k_features}"
        )
...
class _WrappedGroupSelector:
    ...
    def _get_importance(self, subset: Any, result: Dict[str, Any]) -> Any:
        """
