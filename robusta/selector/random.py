...
class RandomSelector(_WrappedSelector):
    ...
    def _fit(self,
        ...
        Args:
            X (pd.DataFrame): the input data with shape (n_samples, n_features)
            y (pd.Series): the target data with shape (n_samples,)
            groups (pd.Series, optional): group labels of samples used for group-wise cross-validation.
...
def binomal_weights(k_min: int, k_max: int, n: int) -> pd.Series:
    ...
    Returns:
        pd.Series: A series of binomial weights for all values of k between k_min and k_max.
...
def uniform_weights(k_min: int, k_max: int) -> pd.Series:
    ...
    Returns:
        pd.Series: A series of uniform weights for all values of k between k_min and k_max.
...
def weighted_choice(weights: pd.Series, rstate: np.random.RandomState) -> int:
    ...
    Args:
        weights (pd.Series): A series of weights for each value of k.
        rstate (numpy.random.RandomState): A random number generator.

    Returns:
        int: A random value of k based on the given weights.
