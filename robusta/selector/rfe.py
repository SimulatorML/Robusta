...
class RFE(_WrappedSelector):
    ...
    def fit(self,
        ...
        Args:
            ...
            groups (Optional[pd.Series], optional): A vector indicating which observations belong to which groups.
    ...
    def partial_fit(self,
        ...
        Args:
            X (pd.DataFrame): A subset of the data.
            y (pd.Series): A subset of the variable.
            groups (Optional[pd.Series], optional): A vector indicating which observations belong to which groups.
    ...
    def k_features_(self) -> int:
        ...
        Returns:
            int: The number of features in the current subset.
    ...
    def forward(self) -> bool:
        ...
        Returns:
            bool: Whether the RFE model is running in forward mode.
...
class ShuffleRFE(RFE):
    ...
    def _eval_subset(self,
        ...
        Args:
            subset (pd.Series): The subset of features to evaluate.
            X (pd.DataFrame): The training data.
            y (pd.Series): The target variable.
            groups (Optional[pd.Series], optional): The group labels for group-wise shuffle. Defaults to None.

        Returns:
            pd.Series: A pandas Series object with the evaluation results.
        """
...
class GroupShuffleRFE(_WrappedGroupSelector, ShuffleRFE):
    ...
    --------
    fit(X, y, groups=None)
        Run group recursive feature elimination with shuffled feature importance
    """
    def _eval_subset(self,
                     subset: pd.Series,
                     X: pd.DataFrame,
        ...
        Args:
            subset: A pandas Series representing a subset of features to evaluate.
            X: A pandas DataFrame representing the input features.
            y: A pandas Series representing the target variable.
            groups: A pandas Series representing the groups for group-aware cross-validation.
...
def _select_k_best(scores: pd.Series,
    ...
    Parameters:
    scores (pd.Series): A Pandas Series containing the feature scores.
    k_best (int): The number of best features to select.
...
def _check_step(step: Union[int, float],
    ...
    Parameters:
    step (Union[int, float]): The step size of the number of features to remove at each iteration.
    n_features (int): The total number of features in the dataset.
    k_features (int): The desired number of features to select.

    Returns:
        int: The validated integer value of the <step> parameter.

    Raises:
        ValueError: If the <step> parameter is not an int or float.
        ValueError: If the <step> parameter is an integer that is less than or equal to 0.
        ValueError: If the <step> parameter is a float that is not in the interval (0, 1).
    """
