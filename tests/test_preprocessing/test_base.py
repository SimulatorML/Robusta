...
from robusta.preprocessing import ColumnGrouper
from robusta.preprocessing import ColumnRenamer
from robusta.preprocessing import ColumnSelector
from robusta.preprocessing import FunctionTransformer
from robusta.preprocessing import Identity
from robusta.preprocessing import SimpleImputer
from robusta.preprocessing import TypeConverter
from robusta.preprocessing import TypeSelector
...
def test_transform_with_string_column(sample_data):
    transformer = ColumnSelector(columns='A')
    transformed_data = transformer.transform(sample_data)
    assert transformed_data.equals(sample_data[['A']])
...
def test_typeselector():
    ...
    pipe = Pipeline([
        ('selector', TypeSelector(dtype=np.number))
    ])
...
def test_identity_transformer():
    identity = Identity()
    X = np.array([[1, 2]])
    assert (X == identity.transform(X=X)).all()
